...
-- N1.ex2.c
select c.company_name, avg(t.amount) from transaction as t
left join company as c on t.company_id=c.id
group by c.company_name
having avg(t.amount) = (select avg(amount) as average from transaction
group by company_id
...
-- N1.ex3.b
select company_id, amount from transaction
...
group by data
/*having total = (select(sum(amount)) as maxim from transaction
group by data
order by maxim desc
limit 1)
; */
...
-- N2.ex2
 select c.country, avg(amount) from company as c
 left join transaction as t on c.id=t.company_id
...
select c.company_name, c.phone, c.country, date(timestamp), t.amount from company c
left join transaction as t on c.id=t.company_id
where (t.amount between 100 and 200) and (date(timestamp) in ("2021-04-29", "2021-07-20", "2022-03-13"))
order by t.amount desc;

-- N3.ex2 count transaccions, group id, especificar 4+ transaccions o -4
select c.company_name, count(company_id),
case 
